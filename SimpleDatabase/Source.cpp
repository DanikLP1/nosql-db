...
void* leaf_node_value(void* node, uint32_t cell_num) {
    return (char*)node + LEAF_NODE_HEADER_SIZE + cell_num * LEAF_NODE_CELL_SIZE + LEAF_NODE_KEY_SIZE;
}
...
void read_input(InputBuffer* input_buffer) {
    fgets(input_buffer->buffer, BUFFER_SIZE, stdin);

    // Удаление символа новой строки, если он был считан
    size_t input_length = strlen(input_buffer->buffer);
    if (input_buffer->buffer[input_length - 1] == '\n') {
